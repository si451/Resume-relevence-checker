...
# API Configuration
# Read from environment only. Do NOT hard-code keys in source.
# Set GROK_API_URL and GROK_API_KEY in a local `.env` file or in your CI secrets.
...
def get_client() -> GrokClient:
    ...
    Note: The client reads API keys from environment variables. If you need to
    initialize with a different key for testing, pass `api_key` when creating
    GrokClient instead of modifying the source file.
    """
    global _client
    if _client is None:
        _client = GrokClient()
